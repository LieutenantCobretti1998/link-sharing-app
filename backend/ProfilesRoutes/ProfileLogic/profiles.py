...
@profiles_bp.route("/change-profile-bio/<int:profile_id>/<string:profile_name>", methods=['PATCH'])
@jwt_required()
def change_profile_bio(profile_id, profile_name):
    user_id = get_jwt_identity()
    data = request.json
    user_instance = UserLogic(db.session)
    if user_id:
        user_allowed_profile = user_instance.check_user_profile_match(user_id, profile_id, profile_name)
        if user_allowed_profile:
            message, code = user_instance.update_profile_bio(profile_id, profile_name, data['new_profile_bio'])
            return jsonify(message), code
        else:
            return jsonify({"error": "Something happened"}), 500
    else:
