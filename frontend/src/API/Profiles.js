...
export const deleteBioDescription = async() => {
    const profile_data = localStorage.getItem("current-profile");
    const parsedProfileData = JSON.parse(profile_data);
    if (!parsedProfileData) {
        throw new Error("Profile is missed!");
    }
     const response = await fetch(`${BACKEND_API_BASE_URL}/delete-bio/${parsedProfileData.profile_id}/${parsedProfileData.profile_name}`, {
         method: "DELETE",
         headers: {
             'X-CSRF-TOKEN': getCSRFToken()
         },
    ...
    if (!response.ok) {
        if (response.status === 401) {
            const refreshed = await refreshAccessToken();
            if (refreshed) {
                const retryResponse = await fetch(`${BACKEND_API_BASE_URL}/delete-bio/${parsedProfileData.profile_id}/${parsedProfileData.profile_name}`, {
                         method: "DELETE",
                         headers: {
                             'X-CSRF-TOKEN': getCSRFToken()
                         },
