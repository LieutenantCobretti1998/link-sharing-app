...
function ResetPassword() {
    ...
    const {mutate: updateForgottenPassword, isLoading, isSuccess} = useMutation({
        mutationFn : (data) => updatePassword(token, data.password),
        onSuccess: () => {
        ...
        <main className="flex justify-center items-center h-screen">
            {isSuccess ?
                <div className="mb-10 text-center">
                    <h1 className="text-3xl mb-2 font-instrumentBold"><b>Please return to the login page</b></h1>
                </div>
            : (
                <>
                <div className="flex flex-col gap-5">
                    ...
                    <section className=" xs:w-[350px] sm:w-[500px] xl:w-[700px] bg-white p-[10%] rounded-xl">
                        <div className="mb-10 text-center">
                            <h1 className="text-3xl mb-2 font-instrumentBold"><b>Update a password</b></h1>
                        </div>
                        <form className="flex flex-col gap-4" onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <div>
                                <label htmlFor="confirm" className="block font-instrumentNormal mb-1">Update Password</label>
                                <input type="password"
                                       id="confirm"
                                       minLength="8"
                                       maxLength="12"
                            ...
                            <Button type={"login"} typeForm={true}>{isLoading ? <MiniSpinner/> : "Update password"}</Button>
                        </form>
                    </section>
                </div>
                <Toaster/>
